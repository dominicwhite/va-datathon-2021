...
datathon_df %>%
  ggplot() +
  geom_sf(aes(fill = lasnap10)) +
  labs(title = "Housing Units Recieving SNAP Benefits that are 10 or More Miles From a Supermarket)")
```

```{r}
datathon_df %>%
  ggplot() +
geom_sf(aes(fill = SNAPS17)) +
  labs(title = "SNAP stores")
```
```{r}
datathon_df %>%
  ggplot() +
geom_sf(aes(fill = SNAPSPTH17)) +
  labs(title = "SNAP stores per 1000 pop")
```
```{r}
datathon_df %>%
  ggplot() +
geom_sf(aes(fill = PCH_SNAPSPTH_12_17)) +
  labs(title = "Percent Change in SNAP stores per 1000 pop (2012-2017)")
```
```{r}
datathon_df %>%
  ggplot() +
geom_sf(aes(fill = `PCH_SNAP_12_17*`)) +
  labs(title = "Percent Change in SNAP Participants per 1000 pop (2012-2017)")
```