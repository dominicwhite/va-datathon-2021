...
snap_vars <- str_c("S2201_C01_", snap_vars)
...
```{r}
va_acs_sf <- left_join(select(filter(va_vehicles_acs, variable == "B08201_001"), GEOID, geometry), va_acs_wide)
```
...
dict_snap <- v2019_subject %>%
  filter(str_detect(name, "^S2201_C01"))
dict_vehic <- v2019 %>%
